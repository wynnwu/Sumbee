...
only); requiring a heavy offline diarizer (pyannote full pipeline) — not real-time, Python-bound.
...
**Decision**: **On-device only** for capture/ASR/diarization — no network in this pipeline (SC-004).
Permissions: microphone (`NSMicrophoneUsageDescription`); audio-capture TCC for the tap; (SCK fallback
would add Screen Recording). Each requested with an explainer; denial → graceful degrade (FR-012). A
...
flip the gate to `.needsLicense` after ≈20 and add StoreKit — **no other code changes**.
